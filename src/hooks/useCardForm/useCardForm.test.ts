...
describe("useCardForm 훅 테스트", () => {
  ...
  test("카드 번호를 올바르게 업데이트해야 한다.", async () => {
    const { result } = renderHook(() => useCardForm());
  ...
  test("숫자가 아닌 값으로 카드 번호를 업데이트하면 업데이트가 되지 않는다.", async () => {
    const { result } = renderHook(() => useCardForm());
  ...
  test("만료 월을 올바르게 업데이트해야 한다.", async () => {
    const { result } = renderHook(() => useCardForm());
  ...
  test("잘못된 만료 월로 업데이트하면 업데이트가 되지 않는다.", async () => {
    const { result } = renderHook(() => useCardForm());
  ...
  test("사용자 이름을 올바르게 업데이트해야 한다.", async () => {
    const { result } = renderHook(() => useCardForm());
  ...
  test("카드 이름과 색상을 올바르게 업데이트해야 한다.", async () => {
    const { result } = renderHook(() => useCardForm());
